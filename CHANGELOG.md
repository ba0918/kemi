...
- 見たの印やコメントを保存するたびにレビューの写しごと書き直していたのをやめ、
  ...
  レビューのメタデータの大きさにも左右されなくなる。10,000 ファイル・コメント 200 件の
  レビューで測ると、これまでの形で毎回書き直していた部分（凍結したレビューの一覧を
  含む）は約 5.2 MB、新しい `<id>.session` は約 150 KB で、およそ 34 分の 1 になる。
  その後ろに続けて毎回書き直していた写し本体（gzip した変更ファイルの内容）は、
  まるごと書かなくなる。
