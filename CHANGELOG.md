...
- 見たの印やコメントを保存するたびにレビューの写しごと書き直していたのをやめ、
  `<id>.session` だけを書くようにした。1 回の保存の大きさが、レビューのファイル数にも
  写しの大きさにも左右されなくなる（10,000 ファイルのレビューで、1 回あたり約 4.9 MB
  から約 129 KB へ）。
...
- 写しのファイルが消えたセッションが `kemi --resume` の一覧に出て、選んでも開けなかった
  のを、一覧に出さないようにした。id を指定した復元は理由を出して終了コード 2 で終わる。
- セッションを消したときに写しのファイルが残ることがあったのを直した。どの
  `<id>.session` からも指されていない `<id>.payload` は掃除で回収する。セッション 1 つの
  大きさも、2 つのファイルの合算で数える。
