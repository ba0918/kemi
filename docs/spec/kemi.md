...
- manifest を省略した場合、`--from` / `--worktree` / `--staged` / `--result` の
  いずれかが必要。
- どれも無い場合は使い方を stderr に出して終了コード 2。
- `--result` と一緒に使えるのは `--any` と `--workspace` だけで、それ以外のフラグや
  入力モード（manifest / `--from` / `--worktree` / `--staged`）との同時指定、
...
- セッション状態（コメント、見た、折りたたみ、解決）はサーバのメモリに置く。
  プロセス終了で消える（`R-SUBMIT`）。例外は submit を確定した結果の JSON で、
  結果ファイルに残す（`R-RESULT`）。ブラウザの localStorage は表示の好み
  （表示モード・テーマ）と下書きにのみ使う。
...
- 終了コード: `0` 承認 / `1` 変更要求 / `2` 起動・使い方・実行時のエラー /
  ...
  stdout に JSON を出さない。ただし、裏でのグループ単位の作成の失敗（`R-UNIT`）、
  由来の計算の失敗（`R-ORIGIN`）、結果ファイルの保存の失敗（`R-RESULT`）はレビューを
  終えず、終了コードも変えない。
- 出力する JSON の契約:
