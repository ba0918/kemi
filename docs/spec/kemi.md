...
- ページ上部に `title` / `subtitle` / `meta` を表示し、表示モード・折返し・
  ...
  ツールチップでまとめる。
- グループ見出しには `title` と `why`（意図）を表示する。見出しは、説明
  （`why` と「見てほしい点」）を残してタイトル行だけに畳める。
- ファイルヘッダに、パスのコピーと全行展開の操作を持つ。
...
- グループ見出しに `why` と「見てほしい点」が出る。見出しの畳みで説明が隠れ、
  タイトル行だけになる。
- 長いコメント本文が既定で畳まれ、その場で展開できる。
- ツリーのグループとディレクトリが開閉できる。
- ファイルヘッダのパスコピーと全行展開が動く。
- 行の `+` からコメントを追加でき、その行の直下に表示される。
- 常設のコメントパネルが存在しない。文字ラベル付きのツールバーが無い。
...
- グループの `why` と `watch` をグループ見出しの下に表示する。`watch` は
  「見てほしい点」として強調する。
...
- 時間の上限（1 秒）は開発機でのスモーク基準とする。計測の対象は
  `scripts/gen-fixture.sh` が作る標準フィクスチャで、追加主体の worktree は
