...
- ページ上部に `title` / `subtitle` / `meta` を表示し、表示モード・折返し・
  ...
  「最終形 | コミットごと」の文字付き 2 択でタイトルの右に置く（`R-UNIT`）。
...
- 「現在」は、表示領域の上端から少し下に引いた基準線（距離は実装が決める）を最後に
  ...
  「最後の変更です」）。`p` は、そこへ送ると実際に表示が上へ戻る止まる場所のうち
  最後のものへ移り、どれも戻らなければ前のファイルの最後の止まる場所へ移る（最初の
  ファイルでは「最初の変更です」）。ファイルの途中では、これは現在の 1 つ前と同じ。
- 表示中のファイルの最後の止まる場所で `n` を押すと、次のファイルの最初の止まる
  ...
  飛ばし、見たのファイルは飛ばさない。最後のファイルの最後の止まる場所では移らず、「最後の変更です」と出す。
  `p` はこれと対称で、最初のファイルの最初の止まる場所で「最初の変更です」と出す。
...
- 「現在」と `n` / `p` の移り先を求める純粋関数のテストで、ファイルの途中、末尾まで
  ...
  止まる場所の扱いが上のとおりになる。
- 最後のファイルの最後の止まる場所で `n` を押すと、移らずに「最後の変更です」と
  出る（人が確認）。
...
- 見たは、ファイルヘッダの文字付きのチェック「見た」とキー `v` で付け外しする。
...
- グループの `watch` を「見てほしい点」としてグループ見出しに常に強調して表示する。
...
- 結果ファイルは全リポジトリ合計で新しい方から 20 件を残し、書くときにそれより古い
  ものを消す。
- 保存に失敗しても、stderr に警告を出し、完了画面に「保存できませんでした」と出す
