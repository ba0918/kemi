...
- Status: 承認済み（2026-09-10）。UX 改訂 v2（2026-09-11）は承認待ち
- 用語の定義: `CONTEXT.md`
- 改訂 v2 の対話の記録: `.agents/tmp/brainstorm-kemi-ux-v2.md`（承認後に削除）
...
- 行コメントは、対象範囲の最後の行の直下に、コードの列の位置から始まる吹き出しで
  ...
  すべて見せる。付けた直後のコメントは開いた状態で出す。開閉の状態はページを
  開いている間だけ覚える。吹き出しの本文の表示幅は約 72 文字に制限し、超える行は
...
- 終了コード: `0` 承認 / `1` 変更要求 / `2` 起動・使い方・実行時のエラー /
  ...
  stdout に JSON を出さない。ただし、裏でのグループ単位の作成の失敗（`R-UNIT`）と
  結果ファイルの保存の失敗（`R-RESULT`）はレビューを終えず、終了コードも変えない。
- 出力する JSON の契約:
