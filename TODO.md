# 残課題

最終更新: 2026-09-11（UX 改訂 v2 の実装時点）

## レビューで記録のみになった指摘（修正要求ではない）

kemi v1（main にマージ済み）:

- id 12: ハイライトのファイル単位の手動指定が再読込で消える
- id 16: A/D が約 5 万件を超える worktree で `git` の引数長（E2BIG）で起動できない
- id 26: digest のグループ・ディレクトリ上限が、余裕があっても常時適用される
- id 29: 離れた 2 つの変更があるファイルで、ハンク折りたたみの中間行が反転する（既存バグ）。
  間の等しい行の並びで、前後の文脈を隠して真ん中を出している（`collapse` の中間の並び）。
  UX 改訂 v2 で折りたたみ行に下の範囲の行番号を出したので、目に付きやすくなった
- id 36: split 表示の replace 旧側に `+` が無く、旧側に行コメントを付けられない
- id 39: コメント応答待ちに新しく開いたエディタが、先行応答の到着で閉じられる（狭い経路）

UI 改訂（main にマージ済み）:

- id 5: 畳みトグルをキーボードで操作すると、再描画でフォーカスが body に落ちる。
  UX 改訂 v2 でグループ帯の説明の開閉は描き直さずに切り替えるようにしたが、キーボードでの
  確認はしていない

UX 改訂 v2 で直したもの: id 28（中断の e2e がサーバの応答を待ってから SIGINT を送る）、
id 42（README の digest の例に `top_files_omitted`）、UI 改訂の id 4（説明の無いグループには
開閉の操作を出さない）。

evidence と oracle の詳細はローカルの `.agents/artifacts/reviews/kemi-v1.json` と
`.agents/artifacts/reviews/kemi-ui-fix.json` にある（git 管理外・この環境のみ）。

## UX 改訂 v2 で決めたこと・残したこと

- 由来（`R-ORIGIN`）の「削除した行のコミットを辿れる」という仮説は、`git blame --reverse`
  だけでは成り立たなかった。横のブランチで消した行がマージで入ると、reverse blame は何もして
  いないマージのせいに見せる。マージの子に当たったら、もう片方の親の側でその行が消えたかを
  マージ基点からたどり直して、どちらの親にも行が残っていたときだけマージ自身を由来にしている。
  たどる深さの上限（8）を超えた行や、たどれない行は「特定できない」にする
- 新側の行を持つ変更ブロックでも、範囲内のコミットに当たらない行が一部あれば「一部特定できない」を
  添えている。仕様がこの文言を明記しているのは削除だけのブロックについてで、新側への広げ方は
  実装で決めた
- 構文ハイライトの配色（syntect のテーマ）はテーマのプリセットと別で、light / dark の 2 つ
  しかない。solarized のプリセットでもハイライトの色は light / dark のまま
- 画面の文言の `JSON` は、データ形式の名前としてコードと同じ扱いで英語のまま出している
  （`R-DIST` の一覧に名前は無い）

## 納品後の環境更新

- `~/.local/bin/diff-review`（旧 Python ツール）を消す。kemi は `cargo install --path .`
  で `~/.cargo/bin/kemi` に入れてあり、リリース後は mise に置き換える
- `diff-review-viewer` スキルは kemi 向けに更新済み（バックグラウンドで起動して終了を
  待つ、stdout の JSON 契約、suggestion はエージェントが適用する）。UX 改訂 v2 の実装後に
  `kemi --result` の案内と、`--group-by` の既定が最終形に変わったことを足す

## リリース

- `v*` タグを切って GitHub Releases に配布する（`ba0918-release`）。`--group-by` の既定が
  `commit` から `file` に変わるのは互換性が崩れる変更なので、次のリリースで扱う
- `mise use -g github:ba0918/kemi` の実アセット確認もその時に行う
- Windows 対応は将来の検討
